...
import queryEvaluator.TwigListD_bup_bas;

public class TwigListDMain_bup_bas {
	...
	public TwigListDMain_bup_bas(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		String suffix = ".csv";
		String fn = queryFN.substring(0, queryFN.lastIndexOf('.'));
		outFileN = Consts.OUTDIR + "sum_"+ fn+"_bupbas++"+suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigListDPro_bup_bas");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				System.out.println("\nEvaluating query " + Q + " ...");
				TwigListD_bup_bas tld = new TwigListD_bup_bas(query, invLsts, bfl);
	...
	public static void main(String[] args) {
		...
		TwigListDMain_bup_bas psMain = new TwigListDMain_bup_bas(dataFileN, queryFileN);
		psMain.run();
