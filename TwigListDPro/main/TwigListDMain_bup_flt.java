...
import queryEvaluator.TwigListD_bup_flt;


public class TwigListDMain_bup_flt {
	...
	public TwigListDMain_bup_flt(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		String suffix = ".csv";
		String fn = queryFN.substring(0, queryFN.lastIndexOf('.'));
		outFileN = Consts.OUTDIR + "sum_"+ fn+"_bupflt"+suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigListDPro_bup_flt");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries_tree.size(); Q++) {
				...
				TwigListD_bup_flt tld = new TwigListD_bup_flt(query_tree, invLsts_reduced, bfl);
	...
	public static void main(String[] args) {
		...
		TwigListDMain_bup_flt psMain = new TwigListDMain_bup_flt(dataFileN, queryFileN);
		psMain.run();
