...
import queryEvaluator.TwigListD_sim_bas;


public class TwigListDMain_sim_bas {
	...
	public TwigListDMain_sim_bas(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		String suffix = ".csv";
		String fn = queryFN.substring(0, queryFN.lastIndexOf('.'));
		outFileN = Consts.OUTDIR + "sum_"+ fn+"_simbas"+suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigListDPro_sim_bas");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				System.out.println("\nEvaluating query " + Q + " ...");
				TwigListD_sim_bas tld = new TwigListD_sim_bas(query, invLsts,  bfl);
	...
	public static void main(String[] args) {
		...
		TwigListDMain_sim_bas psMain = new TwigListDMain_sim_bas(dataFileN, queryFileN);
		psMain.run();
