...
public class TwigStackD_mj {
	...
	public TwigStackD_mj(TreeQuery query, ArrayList<ArrayList<Node>> invLsts, SSPIndex SSPI) {
	...
	public QueryEvalStat run() {
		...
		System.out.println("Time on TwigStackD_SSPI_mj:" + matm + "sec.");
		tt.Start();
		...
		QueryEvalStat stat = new QueryEvalStat(matm, entm, jntm, numSolns);
...
*/
	...
	private void initCursors() {
		...
		for (int i = 0; i < nodes.length; i++) {
			QNode n = nodes[i];
			ArrayList<Node> invLst = mInvLsts.get(n.lb);
