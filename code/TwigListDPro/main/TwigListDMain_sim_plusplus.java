...
import queryEvaluator.TwigListD_sim_plusplus;



public class TwigListDMain_sim_plusplus {
	...
	public TwigListDMain_sim_plusplus(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		String suffix = ".csv";
		String fn = queryFN.substring(0, queryFN.lastIndexOf('.'));
		outFileN = Consts.OUTDIR + "sum_"+ fn+"_sim++"+suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigListDPro_sim_plusplus");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				System.out.println("\nEvaluating query " + Q + " ...");
				TwigListD_sim_plusplus tld = new TwigListD_sim_plusplus(query, invLsts, invLstsByID, bfl);
	...
	public static void main(String[] args) {
		...
		TwigListDMain_sim_plusplus psMain = new TwigListDMain_sim_plusplus(dataFileN, queryFileN);
		psMain.run();
