...
import queryEvaluator.TwigListD_sim_flt;




public class TwigListDMain_sim_flt {
	...
	public TwigListDMain_sim_flt(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		String suffix = ".csv";
		String fn = queryFN.substring(0, queryFN.lastIndexOf('.'));
		outFileN = Consts.OUTDIR + "sum_"+ fn+"_simflt"+suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "TwigListDPro_sim_flt");
	}
	...
	private void evaluate() {
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				TwigListD_sim_flt tld = new TwigListD_sim_flt(query_tree, invLsts_reduced, bfl);
	...
	public static void main(String[] args) {
		...
		TwigListDMain_sim_flt psMain = new TwigListDMain_sim_flt(dataFileN, queryFileN);
		psMain.run();
